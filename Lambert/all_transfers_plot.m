...
text(tmpcirc(1,tmp)+shim,tmpcirc(2,tmp),'$$H_1$$','HorizontalAlignment',...
...
tmp = round(mean([cind4,cind3]));
text(tmpcirc(1,tmp),tmpcirc(2,tmp),'$$H_2$$','HorizontalAlignment',...
    'left','VerticalAlignment','top','Color',[85,107,47]/255)
tmp = round(mean([cind4,cind5]));
text(tmpcirc(1,tmp),tmpcirc(2,tmp),'$$E_4$$','HorizontalAlignment',...
    'center','VerticalAlignment','top','Color','r')
tmp = round(mean([cind6,cind5]));
text(tmpcirc(1,tmp),tmpcirc(2,tmp),'$$E_3$$','HorizontalAlignment',...
    'right','VerticalAlignment','top','Color','b')
tmp = (r1v+r2v)/2;
text(tmp(1),tmp(2),'$$v = \infty$$','HorizontalAlignment',...
    'center','VerticalAlignment','bottom')
[~,ind] = min(abs(rp2(1,:) - tmp(1)));
text(rp2(1,ind),rp2(2,ind),'$$v = v_{esc}$$','HorizontalAlignment',...
    'center','VerticalAlignment','bottom','Color',[255,140,0]/255)
[~,ind] = min(rp1(2,:));
text(rp1(1,ind),rp1(2,ind),'$$v = v_{esc}$$','HorizontalAlignment',...
    'left','VerticalAlignment','top','Color',[85,107,47]/255)
[~,ind] = max(rE1(2,:));
text(rE1(1,ind),rE1(2,ind),'$$v = v_{min}$$','HorizontalAlignment',...
    'center','VerticalAlignment','bottom','Color','b')
[~,ind] = min(rE2(2,:));
text(rE2(1,ind),rE2(2,ind),'$$v = v_{min}$$','HorizontalAlignment',...
    'center','VerticalAlignment','top','Color','r')
...
for j = 1:length(ax.Children)
    if strcmp(ax.Children(j).Type,'line') && strcmp(ax.Children(j).Marker, '.')
        ptinds(end+1) = j;
    end
end
uistack(ax.Children(ptinds),'top')